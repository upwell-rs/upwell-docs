...
import { buildSymbolIndex, type RustdocInput, type Symbol } from '../rustdoc/symbols.ts';
...
import { type CrateInfo, readDirectDependencies, readGit, readToolchain, readSysroot, readWorkspace, runRustdoc } from './workspace.ts';

/** Identity of this generator, recorded in every manifest it writes. */
export const GENERATOR = { name: 'framework-docs-artifact', version: '0.1.0' } as const;
...
export async function generateArtifact(options: GenerateOptions): Promise<GenerateResult> {
	...
	if (!options.reuseRustdoc) {
		report('running rustdoc (nightly, all features) — this takes a while on a cold target directory');
	...
	const directDependencies =
		options.directDependencyCrates === 'workspace' ? await readDirectDependencies(checkout) : [...options.directDependencyCrates];
	...
	const enrichment = sysroot
		? await enrichFromStandardLibrary(sysroot, new Map(index.externals.symbols.map((symbol) => [symbol.path, symbol])), options.standardLibraryCrates)
		: { enriched: new Map(), skipped: 'Could not determine the nightly sysroot.' };

	if (enrichment.skipped) {
		report(`standard library documentation not used — ${enrichment.skipped.split('\n')[0]}`);
	} else {
	...
	const externals = {
		...index.externals,
		symbols: index.externals.symbols.map((symbol) => enrichment.enriched.get(symbol.path) ?? symbol)
	};

	const toolchain = await readToolchain(checkout);
	const repository = workspace.repository.replace(/\/+$/, '');
	const capabilities: ArtifactCapability[] = ['symbols', 'crates', 'search', 'externals'];
	...
	const manifest: ArtifactManifest = {
		...
		contents: { symbols: LAYOUT.symbols, crates: LAYOUT.crates, search: LAYOUT.search, externals: LAYOUT.externals }
	};

	await rm(options.outputDir, { recursive: true, force: true });
	await mkdir(path.join(options.outputDir, LAYOUT.symbols), { recursive: true });

	const shards = await writeShards(path.join(options.outputDir, LAYOUT.symbols), index.symbols);
 ...
 */
async function readRustdocOutput(checkout: string, crates: readonly CrateInfo[]): Promise<RustdocInput[]> {
	...
	for (const crate of crates) {
		...
		inputs.push({ crate: crate.name, file, doc: JSON.parse(contents) as RustdocCrate });
	}
 ...
 */
function featureByCrate(workspace: { facadeCrate: string; crates: readonly CrateInfo[] }): Record<string, string> {
	const facade = workspace.crates.find((crate) => crate.name === workspace.facadeCrate);
 ...
 */
async function writeShards(symbolsDir: string, symbols: readonly Symbol[]): Promise<SymbolShard[]> {
	const byCrate = new Map<string, Symbol[]>();

	for (const symbol of symbols) {
		(byCrate.get(symbol.crate) ?? byCrate.set(symbol.crate, []).get(symbol.crate)!).push(symbol);
	}
 ...
 */
function toSearchEntries(index: { symbols: readonly Symbol[]; paths: Readonly<Record<string, string>> }): SearchEntry[] {
...
function isBetterPath(candidate: string, current: string): boolean {
	...
	return bySegments < 0 || (bySegments === 0 && candidate.length < current.length);
}
