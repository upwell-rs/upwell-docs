...
export function parseExternalIndexFile(value: unknown): ExternalIndexFile {
	...
	return {
		schemaVersion: requirePositiveInteger(root.schemaVersion, 'externals.schemaVersion'),
		symbols: requireArray(root.symbols, 'externals.symbols') as ExternalSymbol[],
		names: requireObject(root.names, 'externals.names') as Record<string, string[]>,
		direct: root.direct === undefined ? [] : requireStringArray(root.direct, 'externals.direct'),
		aliases: root.aliases === undefined ? {} : (requireObject(root.aliases, 'externals.aliases') as Record<string, string>)
	};
}
...
const CAPABILITIES: readonly ArtifactCapability[] = ['symbols', 'crates', 'search', 'externals'];
...
export function parseManifest(value: unknown): ArtifactManifest {
	...
	const schemaVersion = requirePositiveInteger(root.schemaVersion, 'manifest.schemaVersion');
	...
	const capabilities = requireStringArray(root.capabilities, 'manifest.capabilities');
	...
	for (const [index, capability] of capabilities.entries()) {
		if (!CAPABILITIES.includes(capability as ArtifactCapability)) {
			throw new ArtifactSchemaError(`manifest.capabilities[${index}]`, `unknown capability "${capability}"`);
		}
	}

	for (const required of REQUIRED_CAPABILITIES) {
		if (!capabilities.includes(required)) {
			throw new ArtifactSchemaError('manifest.capabilities', `missing required capability "${required}"`);
		}
	}

	if (origin !== 'local' && origin !== 'release') {
		throw new ArtifactSchemaError('manifest.generator.origin', `expected "local" or "release", got "${origin}"`);
	}
	...
	return {
		...
		generator: {
			...
			origin,
			rustdocFormatVersion: requirePositiveInteger(generator.rustdocFormatVersion, 'manifest.generator.rustdocFormatVersion')
		},
		rust: {
			toolchain: requireNullableString(rust.toolchain, 'manifest.rust.toolchain'),
			edition: requireString(rust.edition, 'manifest.rust.edition')
		},
		capabilities: capabilities as ArtifactCapability[],
		sourceLinkTemplate: requireString(root.sourceLinkTemplate, 'manifest.sourceLinkTemplate'),
		contents: {
			symbols: requireString(contents.symbols, 'manifest.contents.symbols'),
			crates: contents.crates === undefined ? undefined : requireString(contents.crates, 'manifest.contents.crates'),
			search: contents.search === undefined ? undefined : requireString(contents.search, 'manifest.contents.search'),
			externals: contents.externals === undefined ? undefined : requireString(contents.externals, 'manifest.contents.externals')
		}
	};
}
...
export function parseSymbolIndexFile(value: unknown): SymbolIndexFile {
	...
	const schemaVersion = requirePositiveInteger(root.schemaVersion, 'symbols.schemaVersion');
	const shards = requireArray(root.shards, 'symbols.shards').map((entry, position) => {
		...
		return {
			...
			count: requirePositiveInteger(shard.count, `symbols.shards[${position}].count`)
		};
	});
	...
	return {
		...
		names: requireObject(root.names, 'symbols.names') as Record<string, string[]>
	};
}

/** Validates one parsed shard: a bare array of symbol records. */
export function parseSymbolShard(value: unknown, file: string): readonly Symbol[] {
...
export function parseCrateIndexFile(value: unknown): CrateIndexFile {
	...
	return {
		schemaVersion: requirePositiveInteger(root.schemaVersion, 'crates.schemaVersion'),
...
export function sourceLink(manifest: ArtifactManifest, file: string, line?: number): string {
	return manifest.sourceLinkTemplate.replaceAll('{path}', file).replaceAll('{line}', String(line ?? 1));
}
