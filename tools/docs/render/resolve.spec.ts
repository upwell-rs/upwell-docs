...
describe('resolveToken', () => {
	const framework = index([
		{ canonical: 'upwell_axum_stomp::Stomp', reachableAt: ['upwell::axum::Stomp'] },
		{ canonical: 'upwell_core::scope::Singleton', reachableAt: ['upwell::Singleton', 'upwell::prelude::Singleton'] },
		{ canonical: 'upwell_di::descriptors::component::Component', reachableAt: ['upwell::prelude::Component'] }
	]);
	...
	it('resolves a token named by an explicit import', () => {
		...
		expect(resolved).toMatchObject({ via: 'import', path: 'upwell::axum::Stomp' });
	});

	it('resolves a token reachable through a glob import', () => {
		const scope = readScope('use upwell::prelude::*;');
		const resolved = resolveToken('Component', scope, framework, { attribute: true });

		expect(resolved).toMatchObject({ via: 'glob', path: 'upwell::prelude::Component' });
	});
	...
	it('prefers an explicit import over a glob', () => {
		...
		expect(resolved).toMatchObject({ via: 'import', path: 'upwell::Singleton' });
	});

	it('resolves an unambiguous name that the snippet never imports', () => {
		const resolved = resolveToken('Stomp', readScope(''), framework);

		expect(resolved).toMatchObject({ via: 'unique', path: 'upwell_axum_stomp::Stomp' });
	});
	...
	it('does not resolve a bare lowercase name the snippet never imports', () => {
		...
		expect(resolveToken('id', readScope('let ticket_id = ticket.id;'), helpers)).toBeUndefined();
	});

	it('resolves a lowercase glob match in attribute position', () => {
		const helpers = index([{ canonical: 'upwell_macros::component', reachableAt: ['upwell::prelude::component'] }]);
		const scope = readScope('use upwell::prelude::*;');

		expect(resolveToken('component', scope, helpers, { attribute: true })).toMatchObject({ via: 'glob' });
	});

	it('does not resolve a lowercase glob match outside attribute position', () => {
		const helpers = index([{ canonical: 'upwell_axum::message', reachableAt: ['upwell::axum::prelude::message'] }]);
	...
	it('still resolves a lowercase name the snippet imported by name', () => {
		const helpers = index([{ canonical: 'upwell_axum::message', reachableAt: ['upwell::axum::message'] }]);
		const scope = readScope('use upwell::axum::message;');

		expect(resolveToken('message', scope, helpers)).toMatchObject({ via: 'import' });
	});

	it('resolves a type-like glob match anywhere', () => {
		const scope = readScope('use upwell::prelude::*;');

		expect(resolveToken('Component', scope, framework)).toMatchObject({ via: 'glob' });
	});

	it('does not resolve an enum variant, which is only ever written qualified', () => {
		const variants = index([
			{ canonical: 'upwell_axum::controller::HttpInputSource::Path', overrides: { kind: 'variant' } }
		]);
	...
	describe('macro invocations', () => {
		...
		it('resolves a lowercase macro reached through a glob import', () => {
			...
			expect(resolveToken('app', scope, index(macros), { macroCall: true })).toMatchObject({
				via: 'glob',
		...
		it('prefers the macro over a module of the same name', () => {
			...
			expect(resolveToken('app', readScope(''), index(macros), { macroCall: true })).toMatchObject({
		...
		it('resolves an attribute name to the macro, never to a module of the same name', () => {
			...
			const both = index([
				{ canonical: 'upwell_axum_macros::controller', reachableAt: ['upwell::axum::prelude::controller'], overrides: { kind: 'proc_macro' } },
				{ canonical: 'upwell_axum::controller', overrides: { kind: 'module' } }
			]);

			expect(resolveToken('controller', readScope(''), both, { attribute: true, attributeName: true })).toMatchObject({
		...
		it('still resolves an attribute argument as the type it is', () => {
			...
			const types = index([{ canonical: 'upwell_axum::scope::HttpRequest', reachableAt: ['upwell::axum::HttpRequest'] }]);

			expect(resolveToken('HttpRequest', readScope(''), types, { attribute: true })).toMatchObject({
		...
		it('resolves nothing for a name whose only match is not a macro', () => {
			...
			expect(resolveToken('Component', scope, framework, { macroCall: true })).toBeUndefined();
		});
	});
});
