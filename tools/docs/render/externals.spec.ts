...
import { type ExternalLookup, externalFromScope, findExternal, readScope, type ResolverIndex } from './resolve.ts';

function external(path: string, kind: string, crate = path.split('::')[0]): ExternalSymbol {
...
describe('findExternal', () => {
	it('resolves a name only one crate defines', () => {
		const index = withExternals([external('tokio::task::JoinHandle', 'struct')]);

		expect(findExternal('JoinHandle', index)).toMatchObject({ crate: 'tokio', kind: 'struct' });
	});
	...
	it('refuses when a direct dependency contests the standard library', () => {
		...
		const index = withExternals(
			[external('std::path::Path', 'struct'), external('axum::extract::Path', 'struct')],
			['axum']
		);
	...
	it('refuses when two non-standard crates share a name', () => {
		const index = withExternals(
			[external('serde_core::ser::Serialize', 'trait'), external('erased_serde::ser::Serialize', 'trait')],
			['serde']
		);
	...
	it('takes the shortest path when one crate re-exports its own item', () => {
		const index = withExternals([external('tokio::sync::Sender', 'struct'), external('tokio::sync::mpsc::inner::Sender', 'struct')]);
	...
	it('lets a primitive win outright, whatever a dependency aliases under the same name', () => {
		...
		const index = withExternals(
			[external('bool', 'primitive', 'std'), external('syn::__private::bool', 'type_alias')],
			['syn']
		);

		expect(findExternal('bool', index)).toMatchObject({ kind: 'primitive', crate: 'std' });
	});
	...
	it('ignores a variant that shares a name with a type', () => {
		...
		const index = withExternals(
			[external('alloc::string::String', 'struct'), external('json::WsValue::String', 'variant')],
			['json']
		);
	...
	describe("a snippet's own imports", () => {
		const framework = withExternals(
			[external('std::path::Path', 'struct'), external('axum::extract::path::Path', 'struct')],
			['axum'],
		...
		it('resolves through a glob import of a module that re-exports the type', () => {
			...
			expect(externalFromScope('Path', readScope('use upwell::axum::prelude::*;'), framework)?.crate).toBe('axum');
		});

		it('resolves through an explicit import naming the item outright', () => {
			expect(externalFromScope('Path', readScope('use std::path::Path;'), framework)?.crate).toBe('std');
		});

		it('says nothing when the snippet imported neither', () => {
			expect(externalFromScope('Path', readScope(''), framework)).toBeUndefined();
		});
	});
	...
	describe('members of an external type', () => {
		const stdlib = withExternals([
			{
				...
				members: [
					{ name: 'len', kind: 'method', signature: 'pub fn len(&self) -> usize', doc: 'Returns the length.', returns: 'usize' },
					{ name: 'new', kind: 'assoc_fn', signature: 'pub fn new() -> String', doc: 'Creates an empty String.', returns: 'String' }
				]
			}
		]);
		...
		it('resolves a method written on a value of that type', () => {
			...
			const { externalMembers } = resolveExpressions(source, readScope(source), stdlib);

			expect([...externalMembers.values()].map((entry) => entry.member.name)).toEqual(['len']);
		});
		...
		it('carries the documentation the standard library wrote', () => {
			...
			expect([...externalMembers.values()][0].member.doc).toBe('Returns the length.');
		});
		...
		it('resolves an associated function written on the type', () => {
			...
			expect([...externalMembers.values()].map((entry) => entry.member.name)).toEqual(['new']);
		});
		...
		it('claims nothing when the type carries no member list', () => {
			...
			const { externalMembers } = resolveExpressions(source, readScope(source), bare);
	...
	describe('Deref', () => {
		...
		const wrappers = withExternals([
			{ ...external('alloc::sync::Arc', 'struct'), derefTarget: 'T', members: [] },
			{
				...external('alloc::string::String', 'struct'),
				derefTarget: 'str',
				members: [{ name: 'capacity', kind: 'method', signature: 'pub fn capacity(&self) -> usize', doc: null, returns: 'usize' }]
			},
			{
				...external('str', 'primitive', 'std'),
				members: [{ name: 'trim', kind: 'method', signature: 'pub fn trim(&self) -> &str', doc: null, returns: '&str' }]
			}
		]);

		it('follows a concrete target, so a String is usable as a str', () => {
			const source = 'fn f(s: String) { s.trim(); }';
			const { externalMembers } = resolveExpressions(source, readScope(source), wrappers);

			expect([...externalMembers.values()].map((entry) => `${entry.owner.name}::${entry.member.name}`)).toEqual(['str::trim']);
		});
		...
		it('substitutes a type parameter from how the receiver was written', () => {
			...
			expect([...externalMembers.values()].map((entry) => entry.owner.name)).toEqual(['String']);
		});
	...
	describe('conversion bounds', () => {
		const paths = withExternals([
			{
				...
				members: [{ name: 'is_dir', kind: 'method', signature: 'pub fn is_dir(&self) -> bool', doc: null, returns: 'bool' }]
			}
		]);
		...
		it('spends an AsRef bound through its method', () => {
			...
			const { externalMembers } = resolveExpressions(source, readScope(source), paths);

			expect([...externalMembers.values()].map((entry) => entry.member.name)).toEqual(['is_dir']);
		});
	...
	describe('type aliases', () => {
		...
		const atomics = withExternals([
			{ ...external('core::sync::atomic::AtomicU64', 'type_alias'), aliasOf: 'Atomic<u64>', members: [] },
			{
				...external('core::sync::atomic::Atomic', 'struct'),
				members: [
					{ name: 'fetch_add', kind: 'method', signature: 'pub fn fetch_add(&self, val: i8) -> i8', doc: null, returns: 'i8', subject: 'Atomic<i8>' },
					{ name: 'fetch_add', kind: 'method', signature: 'pub fn fetch_add(&self, val: u64) -> u64', doc: null, returns: 'u64', subject: 'Atomic<u64>' },
					{ name: 'into_inner', kind: 'method', signature: 'pub fn into_inner(self) -> T', doc: null, returns: 'T' }
				]
			},
		...
		it('follows an alias to the type that has the members', () => {
			const source = 'use std::sync::atomic::AtomicU64;\nfn f(c: AtomicU64) { c.fetch_add(1); }';
			const { externalMembers } = resolveExpressions(source, readScope(source), atomics);

			expect([...externalMembers.values()].map((entry) => entry.member.name)).toEqual(['fetch_add']);
		});
		...
		it('binds a local to what the chosen member returns', () => {
			...
			const { variables } = resolveExpressions(source, readScope(source), atomics);

			expect([...variables.values()].find((entry) => entry.name === 'n')?.path).toBe('u64');
		});
		...
		it('claims nothing when no impl matches the receiver', () => {
			...
			const source = 'use std::sync::atomic::Atomic;\nfn f(c: Atomic<i128>) { c.fetch_add(1); }';
			const { externalMembers } = resolveExpressions(source, readScope(source), atomics);
	...
	describe('enum variants and newtype patterns', () => {
			...
			[
				{
					...
					members: [{ name: 'Relaxed', kind: 'variant', signature: 'Relaxed', doc: null, returns: null }]
				},
				{ ...external('core::cmp::Ordering', 'enum'), members: [] },
				{
					...external('alloc::string::String', 'struct'),
					members: [{ name: 'len', kind: 'method', signature: 'pub fn len(&self) -> usize', doc: null, returns: 'usize' }]
				}
			],
			[],
			{ 'upwell::axum::prelude::Path': 'axum::extract::path::Path' }
		...
		it('resolves a variant written on an external enum', () => {
			const source = 'use std::sync::atomic::Ordering;\nfn f() { let o = Ordering::Relaxed; }';
			const { externalMembers } = resolveExpressions(source, readScope(source), std);

			expect([...externalMembers.values()].map((entry) => entry.member.kind)).toEqual(['variant']);
		});
		...
		it('lets the import pick between two enums of the same name', () => {
			...
			expect([...externalMembers.values()][0].owner.path).toBe('core::sync::atomic::Ordering');
		});
		...
		it('binds a newtype pattern from the annotation rather than the wrapper', () => {
			...
			const source = 'use upwell::axum::prelude::*;\nasync fn h(Path(who): Path<String>) { who.len(); }';
			const { variables, externalMembers } = resolveExpressions(source, readScope(source), std);

			expect([...variables.values()].find((entry) => entry.name === 'who')?.path).toBe('alloc::string::String');
			expect([...externalMembers.values()].map((entry) => entry.member.name)).toEqual(['len']);
		});
		...
		it('binds nothing when the newtype annotation carries no argument', () => {
			...
			expect([...variables.values()].some((entry) => entry.name === 'who')).toBe(false);
		});
	});
