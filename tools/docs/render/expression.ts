...
import type { ExternalMember, ExternalSymbol, Symbol } from '../rustdoc/symbols.ts';
...
const MEMBER_KINDS = new Set(['method', 'assoc_fn', 'assoc_const', 'assoc_type', 'struct_field', 'variant']);
...
function tokenise(code: string): Token[] {
	...
	while (at < code.length) {
		...
		if (character === '"' || (character === 'r' && (code[at + 1] === '"' || code[at + 1] === '#'))) {
...
export function resolveExpressions(
	 ...
	 */
	const annotations = inherited ? merged(inherited, declarations) : declarations;
	...
	const members = walk(tokens, scope, index, bindings, snapshots, externalMembers, written, conversions);
	...
	return {
		members,
		variables: findVariables(tokens, members, bindings, snapshots, typeParameters, index),
		externalMembers,
		declarations,
		declarationSites,
		annotations,
 ...
 */
function withDeclarations(framework: ResolverIndex, declarations: ReadonlyMap<string, Declaration>): ResolverIndex {
...
export function resolveMembers(code: string, scope: SnippetScope, index: ResolverIndex): Map<number, ResolvedToken> {
...
function walk(
	 ...
	 */
	const enclosing: { canonical: string | undefined; rendered: string | undefined }[] = [];
	...
	const setChain = (canonical: string | undefined, rendered?: string, binding?: string) => {
		chain = canonical;
	...
	while (at < tokens.length) {
		...
		if (token.text === 'let') {
			at = readBinding(tokens, at + 1, scope, index, resolved, bindings, snapshots, externalMembers, written, conversions, (found) => (pending = found));
		...
		if (token.text === 'struct' || token.text === 'enum' || token.text === 'union') {
		...
		if (previousText(tokens, at) === '.') {
			at = readMember(tokens, at, chain, chainRendered, chainBinding, conversions, scope, index, resolved, externalMembers, setChain);

			continue;
		}

		// An annotated binding, wherever one can be written: `let x: T`, `fn f(x: T)`, `|x: T|`.
		if (nextText(tokens, at) === ':' && startsBinding(previousText(tokens, at))) {
			at = readAnnotation(tokens, at, scope, index, resolved, bindings, snapshots, externalMembers, written, conversions);
		...
		if (startsBinding(previousText(tokens, at)) && isPatternHead(tokens, at)) {
			...
			const annotated = tokens[parameter.next]?.text === ':' ? writtenType(tokens, parameter.next + 1) : undefined;

			bindPattern(parameter.pattern, undefined, annotated, index, bindings, resolved);
			at = parameter.next;

			continue;
		}

		at = readPath(tokens, at, scope, index, resolved, bindings, snapshots, externalMembers, setChain, written);
	}
...
function findVariables(
	...
	for (const [at, token] of tokens.entries()) {
		...
		const canonical = token.isIdentifier && !typeParameters.has(token.text)
		...
		if (external) {
			variables.set(token.offset, { name: token.text, external, path: canonical, kind: external.kind });
		}
	}
...
function readGenericBounds(
	...
	for (const match of code.matchAll(/\b([A-Z][A-Za-z0-9_]*)\s*:\s*([A-Za-z_][A-Za-z0-9_:]*)\s*(<[^<>]*>)?/g)) {
		...
		const resolved = index.paths[bound] ?? resolveToken(bound.split('::').pop() ?? bound, scope, index)?.symbol.path;
...
function previousText(tokens: readonly Token[], at: number): string | undefined {
...
function isPatternHead(tokens: readonly Token[], at: number): boolean {
	...
	if (!token?.isIdentifier || !/^[A-Z]/.test(token.text) || (opener !== '{' && opener !== '(')) {
		return false;
	}
...
function startsBinding(previous: string | undefined): boolean {
	return previous === '(' || previous === ',' || previous === '|' || previous === 'let' || previous === 'mut';
}
...
function readBinding(
	...
	if (tokens[cursor]?.isIdentifier && tokens[cursor + 1]?.text === ':') {
		return readAnnotation(tokens, cursor, scope, index, resolved, bindings, snapshots, externalMembers, written, conversions);
	}
...
function bindTuple(
	...
	for (const [position, name] of names.entries()) {
		...
		const type = name && element ? typePath(element, canonical ?? '', LOCAL_CRATE, index) : undefined;
...
function readAnnotation(
	...
	const next = readPath(tokens, at + 2, scope, index, resolved, bindings, snapshots, externalMembers, (type) => (annotated = type));
...
function writtenType(tokens: readonly Token[], at: number): string {
	...
	const prefix = tokens[at]?.text === 'impl' || tokens[at]?.text === 'dyn' ? tokens[at].text : '';
...
function readMember(
	...
	if (conversion && conversion.method === token.text) {
		setChain(typePath(conversion.target, chain ?? '', '', index, scope), conversion.target);

		return at + 1;
	}

	const found = chain ? lookUpMember(chain, renderedChain, token.text, scope, index) : undefined;
	...
	if (found.external) {
		...
		setChain(
			returns ? typePath(returns, found.owner, found.external.owner.crate, index, scope) : undefined,
			returns
		);
	...
	resolved.set(token.offset, { symbol: member.symbol, path: member.path, via: 'member' });
	setChain(returnedType(member.symbol, owner, index, scope), rendered(member.symbol));
...
function widen(
	...
	const target =
		symbol?.aliasOf ?? external?.aliasOf ?? symbol?.derefTarget ?? external?.derefTarget;
	...
	const substituted = isTypeParameter(target) ? firstArgumentOf(rendered) : target;
...
function conversionFor(bound: string, argument: string | undefined): Conversion | undefined {
 ...
 */
function substitute(returns: string | null, rendered: string | undefined): string | undefined {
	if (!returns) {
		return undefined;
	}
...
function readPath(
	...
	snapshots: Map<number, string>,
	externalMembers: Map<number, ResolvedExternalMember>,
	setChain: (next: string | undefined, rendered?: string, binding?: string) => void,
	...
	if (path.segments.length === 1) {
		...
		setChain(bound ?? annotatedType([name], scope, index), bound ? written.get(name.text) : undefined, name.text);
	...
	const member = owner ? findMember(`${owner}::${last.text}`, index) : undefined;
	...
	if (!member && path.segments.length === 2) {
		...
		const found = external ? external.members?.find((entry) => entry.name === last.text) : undefined;
		...
		if (external && found) {
			...
			setChain(typePath(found.returns ?? '', external.path, external.crate, index, scope), found.returns ?? undefined);
	...
	if (member) {
		resolved.set(last.offset, { symbol: member.symbol, path: member.path, via: 'member' });
		setChain(returnedType(member.symbol, owner!, index, scope), rendered(member.symbol));
 ...
 */
function canonicalPath(segments: readonly Token[], scope: SnippetScope, index: ResolverIndex): string | undefined {
...
function externalOwner(name: string, scope: SnippetScope, index: ResolverIndex): ExternalSymbol | undefined {
...
function findExternalMember(
	...
	const candidates = type?.members?.filter((entry) => entry.name === name) ?? [];
...
function findMember(path: string, index: ResolverIndex): { symbol: Symbol; path: string } | undefined {
 ...
 */
function returnedType(symbol: Symbol, owner: string, index: ResolverIndex, scope?: SnippetScope): string | undefined {
	if (symbol.kind === 'struct_field' || symbol.kind === 'assoc_const') {
		return typePath(symbol.signature?.split(':').slice(1).join(':') ?? '', owner, symbol.crate, index, scope);
	}

	return symbol.returns ? typePath(symbol.returns, owner, symbol.crate, index, scope) : undefined;
}
 ...
 */
function typePath(rendered: string, owner: string, crate: string, index: ResolverIndex, scope?: SnippetScope): string | undefined {
	const stripped = rendered.trim().replace(/^&\s*(mut\s+)?/, '').replace(/^(impl|dyn)\s+/, '');

	for (const wrapper of TRANSPARENT_GENERICS) {
		if (stripped.startsWith(`${wrapper}<`)) {
			return typePath(firstGenericArgument(stripped), owner, crate, index, scope);
		}
	}

	const head = /^[A-Za-z_][A-Za-z0-9_]*(?:::[A-Za-z_][A-Za-z0-9_]*)*/.exec(stripped)?.[0];
	...
	if (scope) {
		const imported = resolveToken(head, scope, index)?.symbol.path ?? externalFromScope(head, scope, index)?.path;
	...
	const sameCrate = candidates.filter((path) => index.symbols.get(path)?.crate === crate);
