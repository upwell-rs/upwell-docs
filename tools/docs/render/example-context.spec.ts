...
import { exampleContextPreprocessor, resetExampleContext, sharedContextFor } from './example-context.ts';
...
function preprocess(content: string): void {
	exampleContextPreprocessor().markup?.({ content, filename: 'page.svx' } as never);
}
...
describe('exampleContextPreprocessor', () => {
	...
	it('gives a block the declarations of its siblings in the same example', () => {
		...
		expect([...(sharedContextFor(USES)?.keys() ?? [])]).toEqual(['Greeter', 'Greeter::prefix']);
	});
	...
	it('keeps two examples on one page separate', () => {
		...
		expect([...(sharedContextFor('fn go(o: Other) {}')?.keys() ?? [])]).toEqual(['Other', 'Other::value']);
	});
