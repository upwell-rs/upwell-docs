...
function context(): RenderContext {
	...
	const component = symbol('upwell_macros::component', { kind: 'proc_macro', signature: '#[component]' });
...
function renderedText(html: string): string {
	const inner = /<code class="symbol-ref">.*?<(?:a|span)[^>]*>(.*?)<\/(?:a|span)><\/code>/s.exec(html);
...
describe('inlineSymbolPreprocessor', () => {
	...
	it('renders a reference as inline code carrying the symbol metadata', async () => {
		const code = await preprocess('<p>The <Symbol path="upwell::axum::HttpRequest" /> scope.</p>');
	...
	it('shows the last path segment by default', async () => {
		const code = await preprocess('<Symbol path="upwell::axum::HttpRequest" />');

		expect(renderedText(code)).toBe('HttpRequest');
	});

	it('shows the whole path when asked', async () => {
		const code = await preprocess('<Symbol path="upwell::axum::HttpRequest" full />');
	...
	it('accepts a custom label', async () => {
		const code = await preprocess('<Symbol path="upwell::axum::HttpRequest" label="the request scope" />');
	...
	it('fails the build for a symbol that does not exist', async () => {
		...
		// code block here.
		await expect(preprocess('<Symbol path="upwell::Nope" />')).rejects.toThrow(/does not resolve/);
	});

	it('suggests where a mistyped path is actually defined', async () => {
		await expect(preprocess('<Symbol path="upwell::wrong::HttpRequest" />')).rejects.toThrow(
			/defined at: upwell_axum::scope::HttpRequest/
		);
	});

	it('names the page a bad reference is on', async () => {
		await expect(preprocess('<Symbol path="upwell::Nope" />')).rejects.toThrow(/di\.svx/);
	});
	...
	it('escapes braces in documentation, which Svelte would read as an expression', async () => {
		const braces = inlineSymbolPreprocessor(async () => {
			const base = context();
			const withBraces = symbol('upwell_axum::scope::HttpRequest', { doc: 'Use {} for defaults.' });

			return {
				...base,
				index: { ...base.index!, symbols: new Map([['upwell_axum::scope::HttpRequest', withBraces]]) }
			};
		});
