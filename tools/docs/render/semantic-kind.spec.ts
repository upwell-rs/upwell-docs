...
describe('semanticKind', () => {
	it('groups everything callable together', () => {
		expect(['function', 'method', 'assoc_fn'].map(semanticKind)).toEqual(['callable', 'callable', 'callable']);
	});

	it('keeps macros apart from ordinary calls', () => {
		// A macro is not a function, and in Rust the difference changes what the code does.
		expect(['macro', 'proc_macro'].map(semanticKind)).toEqual(['macro', 'macro']);
	});

	it('groups the values a reader writes by name', () => {
		expect(['variant', 'constant', 'assoc_const'].map(semanticKind)).toEqual(['value', 'value', 'value']);
	});
	...
	it('treats every kind of type as one group', () => {
		expect(['struct', 'enum', 'trait', 'type_alias', 'union', 'primitive', 'assoc_type'].map(semanticKind)).toEqual([
