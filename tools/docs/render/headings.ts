...
import { collapse, type IndexedHeading, recordDocument } from './document-index.ts';
...
export function rehypeHeadingAnchors() {
	return (tree: HastNode, file?: { filename?: string; path?: string }): void => {
...
function walk(node: HastNode, used: Map<string, number>, headings: IndexedHeading[], prose: string[]): void {
	const children = (node as { children?: HastNode[] }).children ?? [];

	for (const child of children) {
		if (isElement(child) && (child.tagName === 'h2' || child.tagName === 'h3')) {
...
function decorate(heading: HastElement, used: Map<string, number>): IndexedHeading | undefined {
	const text = textOf(heading);
	const existing = typeof heading.properties.id === 'string' ? heading.properties.id : '';
	...
	heading.properties.class = [heading.properties.class, 'heading'].filter(Boolean).join(' ');
