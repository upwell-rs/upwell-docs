...
const DECLARATION = [
	...
	"\timport { getDocsVersion as __getDocsVersion } from '#lib/docs/version.svelte';",
	'\tconst __docsVersion = __getDocsVersion().id;'
].join('\n');

/** Matches an instance `<script>` — one without `module` or `context="module"`. */
const INSTANCE_SCRIPT = /<script(?![^>]*\b(?:module\b|context\s*=\s*["']module["']))([^>]*)>/;
 ...
 */
export function versionExpressionPreprocessor(): PreprocessorGroup {
	return {
		markup: ({ content }) => {
			...
			if (existing) {
				...
				return { code: content.slice(0, insertAt) + DECLARATION + content.slice(insertAt) };
			}

			return { code: `<script>${DECLARATION}\n</script>\n${content}` };
		}
	};
}
...
export function applyVersionExpression(html: string): string {
	return html.replaceAll(`/docs/${VERSION_SENTINEL}/symbols/`, `/docs/${VERSION_EXPRESSION}/symbols/`);
}
