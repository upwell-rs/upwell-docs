...
export const LANGUAGES = ['rust', 'toml', 'bash', 'shell', 'typescript', 'javascript', 'json', 'yaml', 'text'] as const;
...
export interface RenderContext {
	 ...
	 */
	readonly docsHref?: (path: string) => { href: string; title: string } | undefined;
...
export class UnknownLanguageError extends Error {
	constructor(language: string) {
		super(
			`Unknown code language "${language}".\n\nDocumentation may use: ${LANGUAGES.join(', ')}.\n\nAdd it to LANGUAGES in tools/docs/render/highlight.ts if it is genuinely needed — an unrecognised language is usually a typo, and silently rendering it unhighlighted hides that.`
		);
...
export async function renderCodeBlock(
	...
	return applyVersionExpression(escapeForSvelte(wrap(html, normalised, options)));
}
 ...
 */
export async function highlightInline(code: string, language: Language = 'rust'): Promise<string> {
	const shiki = await getHighlighter();
 ...
 */
function wrap(html: string, language: Language, options: CodeBlockMeta): string {
	...
	const copy = '<button class="code-block__copy" type="button" data-copy aria-label="Copy code">Copy</button>';
 ...
 */
function scrollableRegion(language: Language, options: CodeBlockMeta): ShikiTransformer {
 ...
 */
function symbolTransformer(code: string, context: RenderContext, index: ResolverIndex, block: string): ShikiTransformer {
	const scope = readScope(code);
	const expressions = resolveExpressions(code, scope, index, sharedContextFor(code));
	...
	return {
		...
		span(node, _line, _column, _lineElement, token) {
			...
			if (node.children.length !== 1 || !child || child.type !== 'text' || PROSE_TOKEN.test(child.value)) {
				return;
			}

			const replaced = split(child.value, token.offset, scope, context, expressions, block);
...
type TokenChild =
	...
	| { type: 'element'; tagName: string; properties: Record<string, string>; children: { type: 'text'; value: string }[] };
...
function split(
	...
	const nameAt = attribute ? /[A-Za-z_]/.exec(text.slice(attributeAt + 2))?.index : undefined;
	const attributeNameAt = nameAt === undefined ? undefined : attributeAt + 2 + nameAt;
	...
	for (const match of text.matchAll(IDENTIFIERS)) {
		...
		const resolved = expressions.members.get(at) ?? resolveToken(match[0], scope, expressions.index, position);
		...
		const externalMember = resolved || variable ? undefined : expressions.externalMembers.get(at);
		const external = resolved || variable || externalMember
			? undefined
		...
		const element = declared
			...
			? isLocal(resolved.symbol)
				? localElement(match[0], resolved.symbol, expressions.annotations.get(resolved.path), block)
 ...
 */
function localElement(text: string, symbol: Symbol, declaration: Declaration | undefined, block: string): TokenChild {
...
function symbolElement(text: string, resolved: ResolvedToken, context: RenderContext): TokenChild {
	...
	assign(properties, 'data-symbol-source', context.sourceHref?.(symbol) ?? null);

	if (symbol.deprecation) {
		properties['data-symbol-deprecated'] = symbol.deprecation.note ?? 'This API is deprecated.';
	}
 ...
 */
function variableElement(text: string, variable: ResolvedVariable, context: RenderContext): TokenChild {
	...
	const documented = variable.symbol ? context.docsHref?.(variable.path) : undefined;
	const properties: Record<string, string> = {
		class: 'variable',
		'data-variable': text,
	...
	assign(properties, 'data-variable-href', documented?.href ?? variable.external?.docsUrl);
 ...
 */
function externalFor(token: string, text: string, at: number, scope: SnippetScope, index: ResolverIndex) {
...
const NEVER_RESOLVE_EXTERNAL = new Set([
	'Self', 'Ok', 'Err', 'Some', 'None', 'String', 'Vec', 'Option', 'Result', 'Box'
]);
 ...
 * on this site.
 */
function externalMemberElement(text: string, resolved: ResolvedExternalMember): TokenChild {
	...
	return { type: 'element', tagName: owner.docsUrl ? 'a' : 'span', properties, children: [{ type: 'text', value: text }] };
}
...
function declaredSymbol(declaration: Declaration): Symbol {
	return {
		path: declaration.owner ? `${declaration.owner}::${declaration.name}` : declaration.name,
...
function assign(properties: Record<string, string>, key: string, value: string | null | undefined): void {
	if (value) {
