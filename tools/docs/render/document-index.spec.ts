...
import { collapse, indexedDocuments, MAX_TEXT, recordDocument, resetDocumentIndex } from './document-index.ts';
import { rehypeHeadingAnchors } from './headings.ts';

/** Builds the hast shape mdsvex hands a rehype plugin. */
function element(tagName: string, children: unknown[], properties: Record<string, unknown> = {}) {
...
describe('rehypeHeadingAnchors', () => {
	it('gives headings ids and records them', () => {
		const tree = { type: 'root', children: [element('h2', [text('Declaring a component')])] };
	...
	it('appends an anchor link to each heading', () => {
		...
		const anchor = (tree.children[0] as ReturnType<typeof element>).children.at(-1) as ReturnType<typeof element>;
	...
	it('disambiguates repeated headings, since ids end up in published URLs', () => {
		const tree = { type: 'root', children: [element('h2', [text('Options')]), element('h2', [text('Options')])] };

		run(tree);

		expect(indexedDocuments()[0].headings.map((heading) => heading.id)).toEqual(['options', 'options-2']);
	});

	it('records prose for search', () => {
		const tree = {
			type: 'root',
			children: [element('p', [text('Upwell resolves dependencies at compile time.')])]
		};

		run(tree);

		expect(indexedDocuments()[0].text).toBe('Upwell resolves dependencies at compile time.');
	});
	...
	it('leaves code out of the prose, where it would only add noise', () => {
		const tree = {
			...
			children: [
				...
				element('pre', [element('code', [text('#[component] pub struct Greeter;')])])
			]
		};
...
describe('recordDocument', () => {
	it('orders documents by file, so output is stable across runs', () => {
		...
		expect(indexedDocuments().map((document) => document.file)).toEqual(['/a.svx', '/b.svx']);
	});
});
