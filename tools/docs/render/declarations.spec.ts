...
function declared(code: string): Record<string, string> {
	return Object.fromEntries([...readDeclarations(code)].map(([path, entry]) => [path, entry.kind]));
}
...
describe('readDeclarations', () => {
    ...
    Slow(u32),
		...
		).toEqual({ Mode: 'enum', 'Mode::Fast': 'variant', 'Mode::Slow': 'variant' });
	});
	...
	it('attributes members to the type of an `impl Trait for Type` block', () => {
		...
		expect(declared('impl Component for Greeter {\n    fn configure(&self) {}\n}')).toEqual({
