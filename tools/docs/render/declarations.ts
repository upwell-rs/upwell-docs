...
const ITEM = /^[ \t]*(?:pub(?:\([^)]*\))?[ \t]+)?(?:async[ \t]+|unsafe[ \t]+|const[ \t]+|extern[ \t]+"[^"]*"[ \t]+)*(struct|enum|trait|fn|type|const|mod|union)[ \t]+([A-Za-z_][A-Za-z0-9_]*)/;

/** Matches an `impl` header, capturing the type it is for — the last path before `{` or `where`. */
const IMPL = /^[ \t]*impl(?:<[^>]*>)?[ \t]+(?:.+?[ \t]+for[ \t]+)?([A-Za-z_][A-Za-z0-9_:]*)/;

/** Matches a struct field: `pub name: Type,` at one level of indentation. */
const FIELD = /^[ \t]+(?:pub(?:\([^)]*\))?[ \t]+)?([a-z_][A-Za-z0-9_]*)[ \t]*:[ \t]*(.+?),?[ \t]*$/;
	...
	for (const [at, line] of lines.entries()) {
		...
		if (item) {
			...
			record(declarations, {
				...
				kind: owner && keyword === 'fn' ? (hasSelfReceiver(line) ? 'method' : 'assoc_fn') : ITEM_KINDS[keyword],
			...
			if (opened > closed && (keyword === 'struct' || keyword === 'enum' || keyword === 'trait' || keyword === 'union')) {
			...
			readTypeBodyLine(line, at, offsets, container.name, comment, declarations);
		}
 ...
 */
function record(declarations: Map<string, Declaration>, declaration: Declaration): void {
	const key = declaration.owner ? `${declaration.owner}::${declaration.name}` : declaration.name;
