...
export function externalFromScope(
	...
	if (imported) {
		return external.byPath.get(external.aliases[imported] ?? imported) ?? nearestTo(imported, token, index);
	}
 ...
 * of consulting it.
 */
function nearestTo(imported: string, name: string, index: ResolverIndex): ExternalSymbol | undefined {
	...
	for (const path of index.externals?.byName[name] ?? []) {
		...
		while (
			...
			candidate[candidate.length - 1 - shared] === wanted[wanted.length - 1 - shared]
		) {
			shared += 1;
		}
 ...
 */
const TYPE_KINDS = new Set(['struct', 'enum', 'trait', 'type_alias', 'union', 'primitive']);

export function findExternal(name: string, index: ResolverIndex): ExternalSymbol | undefined {
	 ...
	 */
	const standard = symbols.filter((symbol) => STANDARD_LIBRARY.has(symbol.crate));
	const contested = symbols.some(
		(symbol) => !STANDARD_LIBRARY.has(symbol.crate) && index.externals!.direct.has(symbol.crate)
	);
	...
	// reader recognises.
	return preferred.reduce((best, symbol) => (symbol.path.length < best.path.length ? symbol : best));
}
...
const NEVER_RESOLVE = new Set([
	...
	'use', 'where', 'while',
	'Ok', 'Err', 'Some', 'None', 'Box', 'Vec', 'String', 'str', 'Option', 'Result', 'bool', 'u8',
	'u16', 'u32', 'u64', 'usize', 'i8', 'i16', 'i32', 'i64', 'isize', 'f32', 'f64', 'char'
]);
 ...
 */
const LINKABLE_KINDS = new Set(['struct', 'enum', 'trait', 'function', 'macro', 'proc_macro', 'type_alias', 'module', 'constant']);
...
export function resolveToken(
	...
	const onlyMacros = position.macroCall === true || position.attributeName === true;
	const admits = (symbol: Symbol) => !onlyMacros || MACRO_KINDS.has(symbol.kind);
	const imported = scope.imports.get(token);
	...
	const eligible = onlyMacros
		? candidates.filter((path) => MACRO_KINDS.has(index.symbols.get(path)?.kind ?? ''))
		: candidates;
