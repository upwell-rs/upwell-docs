...
export function recordDocument(document: IndexedDocument): void {
	...
	try {
		...
		writeFileSync(path.join(directory, fileNameFor(document.file)), JSON.stringify(document), 'utf8');
	} catch {
...
function hydrate(): void {
	...
	for (const entry of entries) {
		...
		try {
			const document = JSON.parse(readFileSync(path.join(storeDir(), entry), 'utf8')) as IndexedDocument;
