 ...
 */
async function render(path: string, attributes: SymbolAttributes, context: RenderContext): Promise<string> {
 ...
 */
export function inlineSymbolPreprocessor(context: () => Promise<RenderContext>): PreprocessorGroup {
	return {
		markup: async ({ content, filename }) => {
			...
			for (const match of matches) {
				...
				try {
					code = code.replace(match[0], await render(attributes.path, attributes, resolved));
				} catch (cause) {
					if (cause instanceof InlineSymbolError) {
						throw new InlineSymbolError(`${cause.message}\n\n  Page:\n    ${filename ?? 'unknown'}`);
					}
