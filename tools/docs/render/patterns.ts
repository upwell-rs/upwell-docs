...
export type Pattern =
	...
	| { readonly kind: 'struct'; readonly type: string; readonly fields: readonly PatternField[] }
	/** `Type(inner)` — the name takes the type of the wrapper's first field. */
	| { readonly kind: 'tupleStruct'; readonly type: string; readonly name: string };
...
function readTuple(tokens: readonly Token[], at: number): PatternRead {
	...
	while (cursor < tokens.length && depth > 0) {
		...
		if (depth === 1 && token.isIdentifier && token.text !== 'mut' && token.text !== 'ref') {
			const following = tokens[cursor + 1]?.text;

			names.push(following === ',' || following === ')' ? token.text : undefined);
		}

		cursor += 1;
	}

	return { pattern: names.length > 0 ? { kind: 'tuple', names } : undefined, next: cursor };
}

/** `Type { field, other: renamed, .. }`. */
function readStruct(tokens: readonly Token[], at: number, type: string): PatternRead {
	...
	while (cursor < tokens.length && depth > 0) {
		...
		if (depth === 1 && token.isIdentifier) {
			const renamed = tokens[cursor + 1]?.text === ':' && tokens[cursor + 2]?.isIdentifier;
...
function readTupleStruct(tokens: readonly Token[], at: number, type: string): PatternRead {
	...
	return { pattern: { kind: 'tupleStruct', type, name: inner.text }, next: at + 3 };
}

/** Reads a `::`-joined path, returning its last segment. */
function readPath(tokens: readonly Token[], at: number): { name: string; segments: number; next: number } {
	let cursor = at;
	let segments = 0;
	let name = '';
