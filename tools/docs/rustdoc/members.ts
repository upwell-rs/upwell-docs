...
import { innerKind, isPublic, type RustdocCrate, type RustdocItem } from './types.ts';

/** Item kinds a member walk can contribute: impl and trait bodies, plus struct and enum bodies. */
const MEMBER_KINDS = new Set(['function', 'assoc_const', 'assoc_type', 'struct_field', 'variant']);
...
export function emptyRelations(): ImplRelations {
	return { members: new Map(), implementors: new Map(), traitsByType: new Map(), derefTargets: new Map() };
}
 ...
 */
const INTERNAL_TRAITS = new Set(['StructuralPartialEq', 'StructuralEq', 'Freeze', 'CloneToUninit']);
 ...
 */
export function collectImplRelations(doc: RustdocCrate, crate: string, feature: string | null, into: ImplRelations): void {
	...
	for (const item of Object.values(doc.index)) {
		...
		if (!block || block.blanket_impl || block.is_synthetic || block.is_negative) {
			continue;
		}
		...
		collectBody(doc, block.items ?? [], owner, crate, crateModule, feature, false, into);
	}
...
function derefTargetOf(doc: RustdocCrate, items: readonly number[], crateModule: string): string | null {
	for (const id of items) {
		const item = doc.index[String(id)];

		if (item?.name === 'Target') {
			return qualifyCrateRelative(renderType((item.inner.assoc_type as { type?: unknown } | undefined)?.type), crateModule);
		}
	}
...
function collectTypeBodies(
	...
	for (const [id, item] of Object.entries(doc.index)) {
		...
		if (kind === 'struct') {
			const plain = (item.inner.struct as { kind?: { plain?: { fields?: number[] } } }).kind?.plain;

			collectBody(doc, plain?.fields ?? [], owner, crate, crateModule, feature, false, into);

			continue;
		}

		if (kind === 'enum') {
			collectBody(doc, (item.inner.enum as { variants?: number[] }).variants ?? [], owner, crate, crateModule, feature, false, into);
		}
	}
}
 ...
 */
function recordImplementation(doc: RustdocCrate, reference: { path: string; id: number }, owner: string, into: ImplRelations): void {
...
function collectTraitBodies(
	...
	for (const [id, item] of Object.entries(doc.index)) {
		...
		collectBody(doc, body.items, owner, crate, crateModule, feature, true, into);
	}
}
...
function collectBody(
	...
	for (const id of items) {
		...
		if (!item || !item.name || !kind || !MEMBER_KINDS.has(kind) || !isPublic(item)) {
			continue;
		}
		...
		into.members.set(path, {
			...
			source: item.span ? { file: item.span.filename, line: item.span.begin[0] } : null,
 ...
 */
function returnType(item: RustdocItem, kind: string, crateModule: string): string | null {
	if (kind === 'struct_field') {
		return qualifyCrateRelative(renderType(item.inner.struct_field), crateModule);
	}
...
function memberKind(item: RustdocItem, kind: string): SymbolKind {
	...
	const sig = (item.inner.function as { sig?: { inputs?: [string, unknown][] } } | undefined)?.sig;
...
function subjectId(subject: unknown): number | undefined {
	...
	const resolved = (subject as { resolved_path?: { id?: number } }).resolved_path;
 ...
 */
export function expandMemberAliases(members: ReadonlyMap<string, Symbol>, paths: Map<string, string>): void {
