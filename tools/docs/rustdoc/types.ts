...
export interface RustdocItem {
	...
	visibility: 'public' | 'default' | 'crate' | { restricted: { parent: ItemId; path: string } };
	docs: string | null;
...
export class RustdocFormatError extends Error {
	...
	constructor(file: string, formatVersion: number) {
		super(
			...
			'rustdoc JSON is unstable and changes with the nightly toolchain. Either pin the nightly used to generate artifacts, or update tools/docs/rustdoc/ for the new format.'
		);
