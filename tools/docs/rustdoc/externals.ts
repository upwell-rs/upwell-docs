 ...
 */
export const STANDARD_LIBRARY: ReadonlySet<string> = new Set(['std', 'core', 'alloc', 'proc_macro']);
...
const PRIMITIVE_NAMES = [
	...
	'unit', 'never', 'slice', 'array', 'tuple', 'pointer', 'reference', 'fn'
] as const;
...
export const PRIMITIVES: ReadonlyMap<string, ExternalSymbol> = new Map(
	PRIMITIVE_NAMES.map((name) => [
		name,
		{ path: name, name, kind: 'primitive', crate: 'std', docsUrl: `${PRIMITIVE_ROOT}${name}.html` }
	])
);
 ...
 */
export function collectExternals(doc: RustdocCrate, into: Map<string, ExternalSymbol>): void {
 ...
 */
export function documentationUrl(segments: readonly string[], kind: string, htmlRoot: string | undefined): string | null {
...
export function freezeExternals(
	...
	const symbols = [...merged.values()].sort((a, b) => a.path.localeCompare(b.path));
