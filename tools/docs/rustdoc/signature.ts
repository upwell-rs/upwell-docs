...
export function renderType(type: unknown): string {
	...
	if ('borrowed_ref' in node) {
		const reference = node.borrowed_ref as { lifetime: string | null; is_mutable: boolean; type: unknown };
	...
	if ('dyn_trait' in node) {
		const dynamic = node.dyn_trait as { traits: { trait: { path: string; args?: GenericArgs } }[]; lifetime: string | null };
		const traits = dynamic.traits.map((entry) => `${entry.trait.path}${renderGenericArgs(entry.trait.args ?? null)}`);
	...
	if ('qualified_path' in node) {
		const qualified = node.qualified_path as { name: string; self_type: unknown; trait: { path: string } | null };
	...
	if ('function_pointer' in node) {
		const pointer = node.function_pointer as { sig: { inputs: [string, unknown][]; output: unknown | null } };
		const inputs = pointer.sig.inputs.map(([, argument]) => renderType(argument));
		const output = pointer.sig.output ? ` -> ${renderType(pointer.sig.output)}` : '';
...
function renderGenericArgs(args: GenericArgs): string {
	...
	if ('parenthesized' in args) {
		...
		const output = args.parenthesized.output ? ` -> ${renderType(args.parenthesized.output)}` : '';

		return `(${inputs.join(', ')})${output}`;
	}

	const rendered = args.angle_bracketed.args.map(renderGenericArg).filter((entry) => entry !== '');
...
function renderBounds(bounds: unknown[]): string {
	const rendered = bounds.map((bound) => {
		...
		if ('trait_bound' in node) {
			const traitBound = node.trait_bound as { trait: { path: string; args?: GenericArgs }; modifier: string };
...
function renderGenericParams(generics: unknown): string {
	...
	for (const param of params) {
		...
		if (kind === 'type') {
			const details = node.kind.type as { bounds: unknown[]; default: unknown | null; is_synthetic?: boolean };
			...
			const fallback = details.default ? ` = ${renderType(details.default)}` : '';
...
function renderWhereClause(generics: unknown): string {
	...
	const predicates = (generics as { where_predicates?: unknown[] }).where_predicates ?? [];
	const rendered: string[] = [];
	...
	for (const predicate of predicates) {
		...
		if ('bound_predicate' in node) {
			const bound = node.bound_predicate as { type: unknown; bounds: unknown[] };
 ...
 */
export function qualifyCrateRelative(rendered: string, crateModule: string | undefined): string {
	return crateModule ? rendered.replaceAll('crate::', `${crateModule}::`) : rendered;
}
 ...
 */
export function renderSignature(item: RustdocItem, kind: string, options: SignatureOptions = {}): string | null {
	const rendered = renderItem(item, kind, options);

	return rendered === null ? null : qualifyCrateRelative(rendered, options.crateModule);
}

function renderItem(item: RustdocItem, kind: string, options: SignatureOptions): string | null {
...
function renderFunction(name: string, inner: Record<string, unknown>, visibility: string): string {
	const sig = inner.sig as { inputs: [string, unknown][]; output: unknown | null; is_c_variadic: boolean };
	const header = (inner.header ?? {}) as { is_const?: boolean; is_unsafe?: boolean; is_async?: boolean };
	...
	const inputs = sig.inputs.map(([argumentName, argumentType]) => renderArgument(argumentName, argumentType));
