...
<script lang="ts">
	...
	import type { DocSummary } from '../content/types.ts';
	...
	let { version, previous, next, onsearch }: Props = $props();
	...
	function open(slug: string | undefined): void {
		if (slug) {
			goto(resolve(`/docs/${version.id}/${slug}` as Path));
		}
	}
	...
	createHotkeySequence(['G', 'H'], () => goto(resolve(`/docs/${version.id}` as Path)));
