...
<script lang="ts">
	...
	export function open(): void {
		dialog?.showModal();
		void searchIndex.load(version.id);
		input?.focus();
		input?.select();
	}
	...
	function choose(index: number): void {
		...
		// An undocumented symbol's result points at the framework repository, which is not a route
		// this app can navigate to.
		if (result.record.external) {
