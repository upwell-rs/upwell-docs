...
import type { SymbolKind } from '#tools/docs/rustdoc/symbols';
...
const KEY = Symbol('framework-docs-symbol');

export function setSymbolInfo(symbol: () => SymbolInfo): void {
	setContext(KEY, symbol);
}
...
export function getSymbolInfo(): SymbolInfo {
	const symbol = getContext<(() => SymbolInfo) | undefined>(KEY);

	if (!symbol) {
		throw new Error(
			'No symbol in context. Components that describe a symbol can only be used on a symbol page, under src/content/symbols/.'
		);
	}
