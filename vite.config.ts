...
import { docsManifest } from './tools/docs/content/manifest.ts';
import { exampleContextPreprocessor } from './tools/docs/render/example-context.ts';
import { rehypeHeadingAnchors } from './tools/docs/render/headings.ts';
import { inlineSymbolPreprocessor } from './tools/docs/render/inline-symbol.ts';
import { docsHighlighter, getContext } from './tools/docs/render/mdsvex.ts';
import { versionExpressionPreprocessor } from './tools/docs/render/version-expression.ts';
			...
			compilerOptions: {
				...
				warningFilter: (warning) =>
					!(
						...
						(warning.code === 'a11y_no_noninteractive_tabindex' || warning.code === 'script_context_deprecated')
					)
			},
			adapter: adapter(),
			preprocess: [
				...
				mdsvex({
					...
					highlight: { highlighter: docsHighlighter, optimise: false },
				...
				inlineSymbolPreprocessor(getContext),
		...
		projects: [
...

			{
				...
				test: {
					name: 'server',
					environment: 'node',
					// `tools/` holds the build-time documentation pipeline (region extraction, artifact
					// schema, generator). It is plain Node TypeScript, so it belongs to this project.
