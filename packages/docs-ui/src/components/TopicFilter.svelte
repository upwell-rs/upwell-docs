...
-->
<script lang="ts">
	import type { DocsTopic } from '../types.ts';

	interface Props {
		/** Topics present among the pages of the release being read. */
		available: readonly DocsTopic[];
		active: boolean;
	...
	let { available, active, has, onclear, ontoggle }: Props = $props();
</script>
...
{#if available.length > 1}
	<section class="topics" aria-label="Filter by topic">
		<div class="topics__header">
			...
			{#if active}
				<button class="topics__reset" type="button" onclick={onclear}>
					Clear
				</button>
			{/if}
		</div>
