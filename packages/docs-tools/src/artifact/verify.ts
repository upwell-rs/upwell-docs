...
export function verifyArchiveEntries(
  ...
  if (entries.length > MAX_ENTRIES) {
    throw new ArchiveError(
      "too-many-entries",
      `Archive contains ${entries.length} entries, above the ${MAX_ENTRIES} limit.`,
    );
  }
...
export function assertSafePath(entryPath: string): void {
  ...
  if (entryPath.startsWith("/") || entryPath.startsWith("\\")) {
    throw new ArchiveError(
      "absolute-path",
      `Archive entry "${entryPath}" is an absolute path.`,
      entryPath,
    );
  }
  ...
  if (/^[a-zA-Z]:/.test(entryPath) || entryPath.includes("\\")) {
    throw new ArchiveError(
      "windows-path",
      `Archive entry "${entryPath}" uses a Windows path. Artifact paths are POSIX.`,
      entryPath,
    );
  }
  ...
  if (normalised.split("/").some((segment) => segment === "..")) {
    throw new ArchiveError(
      "traversal-path",
      `Archive entry "${entryPath}" escapes the extraction directory.`,
      entryPath,
    );
  }
}
...
function normalise(entryPath: string): string {
  return entryPath.replace(/^\.\//, "").replace(/\/+/g, "/").replace(/\/$/, "");
}
