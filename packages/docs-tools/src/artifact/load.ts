...
export async function loadArtifact(
  ...
  const manifest = parseManifest(
    await readJson(path.join(root, "manifest.json"), expectedVersion),
  );

  if (manifest.documentation.releaseVersion !== expectedVersion) {
    throw new ArtifactError(
      `Artifact release mismatch.\n\n  Requested: ${expectedVersion}\n  Artifact:  ${manifest.documentation.releaseVersion}\n  Location:  ${root}\n\nRegenerate the artifact, or correct the version in docs.config.ts.`,
    );
  }

  const symbolsDir = path.join(root, manifest.contents.symbols);
  const indexFile = parseSymbolIndexFile(
    await readJson(path.join(symbolsDir, "index.json"), expectedVersion),
  );
  ...
  for (const shard of indexFile.shards) {
    const records = parseSymbolShard(
      await readJson(path.join(symbolsDir, shard.file), expectedVersion),
      shard.file,
    );
  ...
  const crates = manifest.contents.crates
    ? parseCrateIndexFile(
        await readJson(
          path.join(root, manifest.contents.crates),
          expectedVersion,
        ),
      ).crates
    : [];
  ...
  const externals = manifest.contents.externals
    ? parseExternalIndexFile(
        await readJson(
          path.join(root, manifest.contents.externals),
          expectedVersion,
        ),
      )
...
async function readJson(file: string, version: string): Promise<unknown> {
  ...
  try {
    ...
    throw new ArtifactError(
      `Artifact file ${file} is not valid JSON: ${cause instanceof Error ? cause.message : String(cause)}`,
    );
  }
}
...
export function suggestSymbols(
  ...
  const wanted =
    requested.split("::").pop()?.toLowerCase() ?? requested.toLowerCase();
