...
describe("assertSafePath", () => {
  it("accepts an ordinary nested path", () => {
    expect(() =>
      assertSafePath("sources/examples/http/src/greet.rs"),
    ).not.toThrow();
  });
  ...
  it("rejects a parent traversal", () => {
    expect(() => assertSafePath("../../etc/passwd")).toThrowError(
      expect.objectContaining({ rejection: "traversal-path" }),
    );
  });

  it("rejects a traversal buried mid-path", () => {
    expect(() => assertSafePath("sources/../../outside")).toThrowError(
      expect.objectContaining({ rejection: "traversal-path" }),
    );
  });
  ...
  it("rejects an absolute path", () => {
    expect(() => assertSafePath("/etc/passwd")).toThrowError(
      expect.objectContaining({ rejection: "absolute-path" }),
    );
  });

  it("rejects a Windows drive path", () => {
    expect(() => assertSafePath("C:/windows/system32")).toThrowError(
      expect.objectContaining({ rejection: "windows-path" }),
    );
  });
...
describe("verifyArchiveEntries", () => {
  it("accepts a well-formed listing", () => {
    expect(() =>
      verifyArchiveEntries(
        validListing(entry("sources", { type: "directory", size: 0 })),
        1024,
      ),
    ).not.toThrow();
  });
  ...
  it("rejects a symlink rather than resolving it", () => {
    ...
    expect(() => verifyArchiveEntries(listing, 1024)).toThrowError(
      expect.objectContaining({ rejection: "unsafe-entry-type" }),
    );
  });
  ...
  it("rejects an archive that is missing the manifest", () => {
    expect(() =>
      verifyArchiveEntries([entry("symbols/index.json")], 1024),
    ).toThrowError(
      expect.objectContaining({
        rejection: "missing-required-file",
        entry: "manifest.json",
      }),
    );
  });
  ...
  it("rejects a decompression bomb", () => {
    ...
    expect(() => verifyArchiveEntries(listing, 1024)).toThrowError(
      expect.objectContaining({ rejection: "unpacked-too-large" }),
    );
  });
  ...
  it("rejects an archive with too many entries", () => {
    const listing = [
      ...
      ...Array.from({ length: MAX_ENTRIES }, (_, index) =>
        entry(`sources/${index}.rs`),
      ),
    ];

    expect(() => verifyArchiveEntries(listing, 1024)).toThrowError(
      expect.objectContaining({ rejection: "too-many-entries" }),
    );
  });
...
describe("verifyChecksum", () => {
  it("accepts a matching digest regardless of case or surrounding whitespace", () => {
    expect(() =>
      verifyChecksum("ABC123", "  abc123\n", "upwell-docs-0.20.0.tar.zst"),
    ).not.toThrow();
  });

  it("rejects a mismatch and says the archive was not unpacked", () => {
    expect(() =>
      verifyChecksum("abc123", "def456", "upwell-docs-0.20.0.tar.zst"),
    ).toThrowError(/has not been unpacked/);
  });
});
