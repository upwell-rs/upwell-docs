...
function enrich(
  ...
  for (const [id, summary] of Object.entries(doc.paths)) {
    ...
    const existing =
      ...
      (summary.kind === "primitive"
        ? wanted.get(summary.path[summary.path.length - 1])
        : undefined);
  ...
  for (
    let round = 0;
    round < MAX_FOLLOW_ROUNDS && frontier.length > 0;
    round += 1
  ) {
    ...
    for (const { id, base } of frontier) {
      ...
      const aliasOf =
        kind === "type_alias"
          ? renderType((inner as { type?: unknown }).type)
          : null;
      ...
      into.set(base.path, {
        ...
        members: kind
          ? [...membersOf(doc, inner), ...variantsOf(doc, kind, inner)]
          : [],
        derefTarget,
        aliasOf,
      });
      ...
      for (const target of [aliasOf, derefTarget]) {
        const followed = target
          ? follow(target, doc, idsByPath, idsByName, into)
          : undefined;
...
function variantsOf(
  ...
  const variants =
    (inner as { variants?: number[] } | undefined)?.variants ?? [];
...
function membersOf(doc: RustdocCrate, inner: unknown): ExternalMember[] {
  ...
  for (const implId of impls) {
    ...
    for (const memberId of block.items ?? []) {
      ...
      collected.push({
        ...
        kind:
          kind === "function"
            ? hasReceiver(item.inner.function)
              ? "method"
              : "assoc_fn"
            : kind,
  ...
  const contested = new Set(
    collected
      .filter(
        (member, at) =>
          collected.findIndex((other) => other.name === member.name) !== at,
      )
  ...
  return collected
    .map((member) =>
      contested.has(member.name) ? member : { ...member, subject: undefined },
    )
...
function returnTypeOf(
  item: { inner: Record<string, unknown> },
  kind: string,
): string | null {
  if (kind === "assoc_const") {
    return renderType(
      (item.inner.assoc_const as { type?: unknown } | undefined)?.type,
    );
  }
...
function derefTargetOf(doc: RustdocCrate, inner: unknown): string | null {
  ...
  for (const implId of impls) {
    ...
    if (
      !block?.trait ||
      block.blanket_impl ||
      block.trait.path?.split("::").pop() !== "Deref"
    ) {
      continue;
    }
...
function hasReceiver(inner: unknown): boolean {
  const sig = (inner as { sig?: { inputs?: [string, unknown][] } } | undefined)
    ?.sig;
