...
export function buildSymbolIndex(
  ...
  const frameworkModules = new Set(
    (options.workspaceCrates ?? inputs.map((input) => input.crate)).map(
      moduleName,
    ),
  );
  const paths = resolveAliases(
    symbols,
    edges,
    externalAliases,
    frameworkModules,
  ...
  return freeze(
    symbols,
    paths,
    freezeExternals(externals, options.directDependencies, externalAliases),
  );
}
 ...
 */
interface MutableSymbol extends Omit<
  Symbol,
  "implementations" | "implementors" | "derefTarget"
> {
  aliases: string[];
  ...
  for (const [id, item] of Object.entries(doc.index)) {
    ...
    symbols.set(path, {
      ...
      aliasOf:
        kind === "type_alias"
          ? qualifyCrateRelative(
              renderType((item.inner.type_alias as { type?: unknown }).type),
              crateModule,
            )
          : null,
    });
 ...
 */
function adoptMembers(
  symbols: Map<string, MutableSymbol>,
  relations: ImplRelations,
): void {
...
function walkModule(
  ...
  for (const childId of module.items) {
    ...
    if (kind === "use") {
      ...
      const summary =
        use.id === undefined ? undefined : doc.paths[String(use.id)];
      ...
      edges.push({
        ...
        isGlob: use.is_glob,
        target: summary
          ? { path: summary.path.join("::"), external: summary.crate_id !== 0 }
          : undefined,
      });
    }
  }
...
function resolveAliases(
  ...
  for (let pass = 0; pass < MAX_ALIAS_PASSES; pass += 1) {
    ...
    for (const edge of edges) {
      added += edge.isGlob
        ? expandGlob(edge, paths, externals)
        : expandDirect(edge, paths);
    }
...
function adoptExternalAliases(
  ...
  for (const edge of edges) {
    if (
      edge.isGlob ||
      !edge.target ||
      paths.has(edge.alias) ||
      externals.has(edge.alias)
    ) {
      continue;
    }
    ...
    if (
      paths.has(edge.target.path) ||
      frameworkModules.has(edge.target.path.split("::")[0])
    ) {
      continue;
    }
...
function expandGlob(
  ...
  let added = 0;
  const sources =
    edge.target && !edge.target.external
      ? [edge.target.path, ...sourceCandidates(edge)]
      : sourceCandidates(edge);
