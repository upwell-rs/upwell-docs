...
export function sharedContextFor(
  code: string,
): ReadonlyMap<string, Declaration> | undefined {
...
function register(region: string): void {
  ...
  const fences = [...region.matchAll(RUST_FENCE)].filter(
    (fence) => !/(?:^|\s)plain(?:\s|$)/.test(fence[1]),
  );
  ...
  const rendered = fences.map(
    (fence) => !/(?:^|\s)context(?:\s|$)/.test(fence[1]),
  );
    ...
    for (const [other, declarations] of declared.entries()) {
      ...
      for (const [path, declaration] of declarations) {
        ...
        if (!context.has(path)) {
          context.set(path, {
            ...declaration,
            block: rendered[other] ? ids[other] : undefined,
          });
        }
      }
    }
