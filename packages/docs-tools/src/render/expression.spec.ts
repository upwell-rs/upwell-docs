...
const FRAMEWORK: readonly TestEntry[] = [
  {
    canonical: "upwell_app::app::App",
    reachableAt: ["upwell::App", "upwell::prelude::App"],
  },
  {
    canonical: "upwell_app::app::AppBuilder",
    reachableAt: ["upwell::AppBuilder"],
  },
  {
    canonical: "upwell_axum::config::AxumConfig",
    reachableAt: ["upwell::axum::AxumConfig"],
  },
  {
    canonical: "upwell_app::app::App::builder",
    overrides: {
      kind: "assoc_fn",
      returns: "AppBuilder<D>",
      signature: "pub fn builder() -> AppBuilder<D>",
    },
  },
  {
    ...
    overrides: {
      ...
      signature: "pub async fn serve(&self) -> Result<(), Error>",
    },
  },
  {
    ...
    overrides: {
      kind: "method",
      returns: "Self",
      signature: "pub fn name(self, name: &str) -> Self",
    },
  },
  {
    ...
    overrides: {
      kind: "method",
      returns: "App<D>",
      signature: "pub fn build(self) -> App<D>",
    },
  },
  {
  ...
  },
  {
    ...
    overrides: {
      kind: "method",
      returns: "Self",
      signature: "fn configure(&self) -> Self",
    },
  },
  {
...
function members(
  ...
  for (const [offset, token] of resolved) {
    found[source.slice(offset).match(/^[A-Za-z_][A-Za-z0-9_]*/)![0]] =
      token.symbol.path;
  }
...
describe("resolveMembers", () => {
  it("resolves an associated function called on a type", () => {
    expect(members("let b = App::builder();")).toMatchObject({
      builder: "upwell_app::app::App::builder",
    });
  });

  it("resolves a method on a binding whose type was annotated", () => {
    expect(members("fn run(app: App) { app.serve(); }")).toMatchObject({
  ...
  it("resolves a chain through the return type of each call", () => {
    ...
    expect(
      members('let app = App::builder().name("demo").build();'),
    ).toMatchObject({
      builder: "upwell_app::app::App::builder",
  ...
  it("carries an inferred type from a let binding to a later statement", () => {
    expect(
      members("let app = App::builder().build();\napp.serve();"),
    ).toMatchObject({
      serve: "upwell_app::app::App::serve",
    });
  });
  ...
  it("resolves a struct field", () => {
    expect(
      members("fn read(config: AxumConfig) { config.port; }"),
    ).toMatchObject({
  ...
  it("keeps the chain alive through the default() convention", () => {
    ...
    expect(
      members("let config = AxumConfig::default();\nconfig.port;"),
    ).toMatchObject({
      port: "upwell_axum::config::AxumConfig::port",
    });
  });
  ...
  it("does not treat a nested argument as a continuation of the outer chain", () => {
    expect(
      members("fn run(app: App) { log(App::builder()); app.serve(); }"),
    ).toMatchObject({
      builder: "upwell_app::app::App::builder",
      serve: "upwell_app::app::App::serve",
    });
  ...
  it("ignores a member named inside a string", () => {
    expect(
      members(
        'fn run(app: App) { let note = "app.serve() is the entry point"; }',
      ),
    ).toEqual({});
  });
  ...
  it("distinguishes two receivers that share a member name", () => {
    ...
    expect(paths).toEqual([
      "upwell_app::app::App::serve",
      "upwell_app::app::AppBuilder::build",
    ]);
  });
  ...
  describe("associated items", () => {
    ...
    it("resolves an associated constant through the type that owns it", () => {
      expect(
        members("fn read(config: AxumConfig) { AxumConfig::port; }"),
      ).toMatchObject({
        port: "upwell_axum::config::AxumConfig::port",
      });
    });
  ...
  describe("generics", () => {
    it("resolves a method on a receiver bounded by a framework trait", () => {
      expect(
        members("fn register<C: Component>(item: C) { item.configure(); }"),
      ).toMatchObject({
    ...
    it("resolves a method on a receiver bounded in a where clause", () => {
      expect(
        members(
          "fn register<C>(item: C) where C: Component { item.configure(); }",
        ),
      ).toMatchObject({
        configure: "upwell_di::descriptors::Component::configure",
      });
    ...
    it("carries the chain through a turbofish", () => {
      ...
      expect(
        members('let app = App::builder().name::<String>("demo").build();'),
      ).toMatchObject({
        build: "upwell_app::app::AppBuilder::build",
      });
    });
    ...
    it("carries the chain through a nested turbofish", () => {
      expect(
        members(
          'let app = App::builder().name::<Vec<String>>("demo").build();',
        ),
      ).toMatchObject({
        build: "upwell_app::app::AppBuilder::build",
      });
    ...
    it("does not bind a parameter whose bound is not a framework trait", () => {
      expect(
        members("fn register<C: Clone>(item: C) { item.configure(); }"),
      ).toEqual({});
    });
  ...
  describe("destructuring", () => {
    ...
    const patterns = testIndex([
      ...FRAMEWORK,
      {
        ...
        overrides: {
          kind: "method",
          returns: "(AxumConfig, App<D>)",
          signature: "pub fn split(self) -> (AxumConfig, App<D>)",
        },
      },
    ]);
    ...
    it("lets a member resolve through a name a tuple pattern bound", () => {
      const source =
        "use upwell::prelude::*;\nfn run(app: App) {\n    let (cfg, _) = app.split();\n    cfg.port;\n}";
      const { members } = resolveExpressions(
        source,
        readScope(source),
        patterns,
      );
    ...
    it("follows a rename, binding the new name rather than the field", () => {
      ...
      expect(found).toMatchObject({
        settings: "upwell_axum::config::AxumConfig",
      });
    ...
    it("types a destructuring parameter, which is how an extractor is written", () => {
      expect(
        locals(
          "fn handle(Holder { config }: Holder) {\n    config.port;\n}",
          withHolder,
        ),
      ).toMatchObject({
  ...
  describe("typed locals", () => {
    ...
    it("annotates a local whose type came from its initialiser", () => {
      ...
      expect(locals("let app = App::builder().build();\napp.serve();")).toEqual(
        ["app: upwell_app::app::App", "app: upwell_app::app::App"],
      );
    });

    it("annotates a local bound through a generic trait bound", () => {
      expect(
        locals("fn go<C: Component>(item: C) { item.configure(); }"),
      ).toEqual([
    ...
    it("does not annotate a generic parameter, which is a type rather than a value", () => {
      expect(
        locals("fn go<C: Component>(item: C) { item.configure(); }"),
      ).not.toContain("C: upwell_di::descriptors::Component");
    });
